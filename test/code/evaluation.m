...
%% caffe setttings
%matCaffe = fullfile('F:/Code/caffe/build/install/matlab');
%addpath(matCaffe);
gpu = 1;
if gpu
   gpu_id = 0;
...
%model   = '../../preprocess/data/caffe_script/sphereface08_deploy.prototxt';
%model   = '../../train/code/dim256/sphereface06_deploy.prototxt';
model   = '../../train/code/mobilenet-dim512/sphereface10_deploy.prototxt';
weights = '../../train/result/mobilenet_dim512_sphereface10_model_iter_40000.caffemodel';
%weights = '../../train/result/dim256_sphereface06_model_iter_96000.caffemodel';
%weights = '../../preprocess/data/result/sphereface06_model_FINE1-90_iter_12000.caffemodel';
net     = caffe.Net(model, weights, 'test');
%net.save('../result/sphereface_model.caffemodel');

%% compute features
pairs = parseList('../data/pairs.txt', fullfile(pwd, '../data/lfw-112X96'));
%pairs = parseList('../data/pairs.txt', fullfile(pwd, '../../preprocess/data/lfw_crop_sphereface'));
...
for i = 1:10
   ...
   % disp(sprintf('threshold=%f',threshold));
    ...
    fprintf('%d\t%2.2f%%\n', i, ACCs(i)*100);
   % save(sprintf('result\\%d_mu.mat',i-1),'mu');
   % valid_scores = scores(valFold);
   % save(sprintf('result\\%d_validscores',i-1),'valid_scores');
   % test_scores = scores(testFold);
   % save(sprintf('result\\%d_testscores',i-1),'test_scores');
end
fprintf('----------------\n');
fprintf('AVE\t%2.2f%%\n', mean(ACCs)*100);
